...
def build_argparser():
    ...
    default_paths = {
        ...
        'i':r'C:\Users\DZERICHARD\Desktop\Udacity Projects\ComputerPointerController\bin\demo.mp4',
...
def main():
    ...
    for n, frame in Feed.next_batch():
        ...
        face_crop, face_coords = face_det.predict(frame)
        if len(face_coords) == 0:
            log.error("No face detected")
            continue

        hp_angles = head_pose.predict(face_crop)
        l_eye, r_eye, eye_coords = face_landmark.predict(face_crop)
        mouse_coords, gaze_vector = gaze_est.predict(frame, l_eye, r_eye, hp_angles)
